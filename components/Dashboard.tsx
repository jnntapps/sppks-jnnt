...
import { Users, Briefcase, Building2, Printer, PieChart, Smartphone, X, Share } from 'lucide-react';
    ...
    <div className="space-y-6 animate-fade-in print:p-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-4 gap-4 print:hidden">
        ...
        <div className="flex gap-2">
            <button 
                ...
                className="flex items-center gap-2 px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-sm font-medium transition-colors border border-blue-200"
            >
                <Smartphone size={18} /> App di HP
            </button>
            <button 
                onClick={handlePrint}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Jumlah Staf</p>
            ...
            {/* Card 2: In Office */}
            <div className="bg-white rounded-xl shadow-sm p-6 border-l-4 border-emerald-500 flex items-center justify-between">
            <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Dalam Pejabat</p>
                <p className="text-3xl font-bold text-emerald-600 mt-1">{inOffice}</p>
            </div>
            ...
            <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Keluar</p>
                <p className="text-3xl font-bold text-red-600 mt-1">{outOffice}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 flex flex-col items-center justify-center border border-slate-100 print:border-slate-300">
            ...
            <div className="relative w-32 h-32">
                ...
                <div className="absolute inset-2 bg-white rounded-full flex items-center justify-center flex-col">
                    <span className="text-xl font-bold text-slate-800">{Math.round(inOfficePercent)}%</span>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden mt-6 border border-slate-200">
        <div className="bg-slate-50 px-6 py-4 border-b border-slate-200">
          <h3 className="font-semibold text-slate-700">Status Semasa Pegawai</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600">
            ...
            <tbody className="divide-y divide-slate-100">
              {staffList.map((staff) => (
                <tr key={staff.id} className="hover:bg-slate-50 transition-colors break-inside-avoid">
                  <td className="px-6 py-3 font-medium text-slate-800">{staff.name}</td>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white rounded-xl shadow-2xl max-w-md w-full overflow-hidden">
                <div className="bg-blue-600 p-4 flex justify-between items-center text-white">
                    <h3 className="font-bold flex items-center gap-2"><Smartphone size={20}/> Pasang di Telefon</h3>
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg text-xs text-blue-800">
                        Aplikasi ini akan muncul di skrin utama telefon anda seperti aplikasi biasa untuk akses pantas.
                    </div>
                    <button 
